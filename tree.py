...
class Tree(object):
    ...
    def add_sibling(self):
        """ Add sibling label to current node's label"""
        nodes = list(self.bottomup())
        for node in nodes:
            sib = ""
            LabelNew = ""
            if len(node.children) > 1 and node.parent and '*' in node.label:
                for ii in node.parent.children:
                    if not ii.label == node.label:
                        sib = ii.label
                LabelNew = node.label + "/" + sib
                node.label = LabelNew
    ...
    def add_parent(self):
        """ Add parent label to current node's label"""
        nodes = list(self.bottomup())
        for node in nodes:
            par = ""
            LabelNew = ""
            if len(node.children) > 1 and node.parent and '*' in node.label:
                LabelNew = node.label + "$" + node.parent.label
                node.label = LabelNew
                
    def remove_sibling(self):
        """ Remove sibling label to current node's label"""
        nodes = list(self.bottomup())
        for node in nodes:
            if len(node.children) > 1 and node.parent and "/" in node.label :
                labNew = []
                labNew = node.label.split("/")
                node.label = labNew[0]
    ...
    def remove_mark_base_NP(self):                
        nodes = list(self.bottomup())
        for node in nodes:
            if "_Base" in node.label:
